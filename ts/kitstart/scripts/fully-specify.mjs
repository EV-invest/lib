...
//
// `tsc` emits each module as written, so `import { x } from "./site"` stays
// extensionless — which Node's ESM resolver and webpack's `fullySpecified`
// (on for `"type": "module"` packages) both refuse. This resolves each
// specifier against the emitted files: `x.js` if it exists, else
// `x/index.js`, and fails the build on anything that is neither. The same
// tool as the kit's unbundled build.
