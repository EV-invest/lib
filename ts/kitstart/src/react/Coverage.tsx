...
export function Coverage<L extends string>({ place, locale, map, head, className }: CoverageProps<L>) {
  ...
  return (
    <div className={cn("flex flex-col gap-6 md:gap-8", className)}>
      ...
      {address && map && <MapFacade query={`${place.gbpName}, ${address}`} title={map.title} show={map.show} address={address} />}
    </div>
  );
}
