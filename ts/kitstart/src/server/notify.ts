...
export function leadNotifier(
  ...
  env: NotifyEnv,
  options: { format?: (lead: Lead, id: number) => LeadMail; log?: Pick<Console, "warn"> } = {},
): LeadNotifier {
  const { brand } = site;
  const log = options.log ?? console;
  const format = options.format ?? ((lead: Lead, id: number) => defaultLeadMail(brand, lead, id));
  let mail: { url: string; from: string; to: string; helo: string } | null = null;
  if (env.smtpUrl) {
    ...
    mail = { url: env.smtpUrl, from, to, helo };
  }
  return {
