 ...
 *   hidden, because the hidden state exists only inside the keyframes.
 ...
 *   `variants`, …) have no meaning here; plain DOM attributes pass through.
 *
...
export function Reveal({
  ...
  return (
    <div data-ev-motion={onMount ? "mount" : "view"} style={{ ...vars, ...style }} {...props}>
      {children}
    </div>
  );
}
...
export function Settle({ duration = DUR.base, style, children, ...props }: SettleProps) {
  ...
  return (
    <div data-ev-motion="settle" style={{ ...vars, ...style }} {...props}>
      {children}
    </div>
  );
}
...
export function Stagger({ delay = 0, step = STAGGER, onMount = false, style, children, ...props }: StaggerProps) {
  ...
  return (
    <div data-ev-stagger={onMount ? "mount" : "view"} style={{ ...vars, ...style }} {...props}>
      {numbered}
    </div>
  );
}
...
export function StaggerItem({ distance = RISE, index = 0, style, children, ...props }: StaggerItemProps) {
  ...
  return (
    <div data-ev-motion="item" style={{ ...vars, ...style }} {...props}>
      {children}
    </div>
  );
}
