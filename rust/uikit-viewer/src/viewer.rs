use dioxus::prelude::*;
use ev_lib::uikit::{
	Alert, AlertDescription, AlertTitle, AlertVariant, Badge, BadgeVariant, Button, ButtonVariant, CalendarDate, Card, CardContent, CardDescription, CardHeader, CardTitle, DateTimePicker,
	LocalDateTime, Separator, Size, Spinner, ToastPosition, ToastVariant, Toaster, ToasterProvider, use_toaster,
};
...
fn Gallery() -> Element {
	...
	let mut moment = use_signal(|| None::<LocalDateTime>);
	// The Rust kit has no clock; the viewer pins "today" so the grid has a highlight.
	let today = CalendarDate::new(2026, 6, 15);
	...
	rsx! {
		div { class: "bg-background text-ink min-h-dvh",
			div { class: "mx-auto max-w-3xl space-y-12 p-6 sm:p-12",
				...
				section { class: "space-y-3",
					h2 { class: "text-ink text-lg font-semibold", "Date & time" }
					div { class: "max-w-xs space-y-2",
						DateTimePicker {
							value: moment(),
							on_change: move |next| moment.set(next),
							min: LocalDateTime::new(today, 0, 0),
							today,
							placeholder: "Pick a moment",
						}
						p { class: "text-ink-soft text-sm",
							match moment() {
								Some(v) => format!("Selected: {:04}-{:02}-{:02} {:02}:{:02}", v.date.year, v.date.month, v.date.day, v.hour, v.minute),
