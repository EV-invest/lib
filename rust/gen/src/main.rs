...
//!   theme.css · ev.css      │                    ├─▶ rust/classes/uikit-classes.txt
//!   tokens.css · motion.css─┘                    ├─▶ rust/classes/css/*.css
...
fn main() {
	...
	for (component, exports) in manifest::manifest() {
		write_module(&root, "uikit", component, &exports);
	}
	for (package, module, exports) in manifest::shared() {
		write_module(&root, package, module, &exports);
	}
...
/// One generated TS module, into the owning package's `src/generated/`.
fn write_module(root: &Path, package: &str, module: &str, exports: &[Ts]) {
	let dir = root.join(format!("ts/{package}/src/generated"));
	...
	let path = dir.join(format!("{module}.ts"));
	fs::write(&path, out).expect("write generated module");
...
///
/// Three sheets, each flat on its own: `theme.css` is the brand-neutral
/// contract (Tailwind mapping, derived tokens, geometry, motion), `ev.css` is
/// EV's palette against it, and `tokens.css` is the two concatenated — what
/// every existing consumer imports. A non-EV consumer takes `theme.css` plus
/// its own palette; the split lives in the sources, never in a shipped
/// `@import`.
fn write_stylesheets(root: &Path) {
	...
	for name in ["tokens.css", "theme.css", "ev.css"] {
...
enum Ts {
	...
	/// A `number` or other non-string literal, written through verbatim.
	Scalar {
		name: &'static str,
		value: String,
	},
...
impl Ts {
	fn render(&self, out: &mut String) {
		match self {
			...
			Ts::Scalar { name, value } => writeln!(out, "export const {name} = {value};").unwrap(),
			Ts::Array { name, items } => {
				let items: Vec<String> = items.iter().map(|i| format!("{i:?}")).collect();
				writeln!(out, "export const {name} = [{}] as const;", items.join(", ")).unwrap();
			}
	...
	fn collect_classes(&self, out: &mut BTreeSet<String>) {
		match self {
			...
			// Not class tables — the shared registries (locales, country codes).
			Ts::Scalar { .. } | Ts::Array { .. } => {}
