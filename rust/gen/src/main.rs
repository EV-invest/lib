...
//! Rust styling source of truth (`ev_lib_classes`), plus the two things a Rust
//! consumer cannot reach inside a crates.io checkout: the Tailwind class
//! inventory it `@source`s, and the token stylesheets it `@import`s. Run via
//! `cargo run -p ev_lib_gen`; the output is committed and CI fails if it drifts
//! (`git diff --exit-code ts/uikit/src/generated rust/classes`).
...
/// The token sheets, flattened, inside `ev_lib_classes` so a Rust consumer's
/// `build.rs` can write them out — the same reason `CLASS_INVENTORY` exists. The
/// TS port gets them by a file copy in its `prepare` script; a crate has no
/// equivalent, and the alternative is what REA did: a hand-kept byte-for-byte
/// copy carrying a "change one, change the other" comment.
///
...
fn write_stylesheets(root: &Path) {
	let dir = root.join("rust/classes/css");
	fs::create_dir_all(&dir).expect("create css dir");
	for name in ["tokens.css", "tokens-legacy.css"] {
		let flat = flatten_css(root, name);
		let path = dir.join(name);
		fs::write(
			&path,
			format!("/* AUTO-GENERATED by `cargo run -p ev_lib_gen` from the repo-root {name} — do not edit. */\n{flat}"),
		)
