...
///
/// #105 cut this on the argument that brand assets are not a UI kit's business.
/// The argument is sound and the move was not: nothing else shipped the faces, so
/// the `--font-*` chains in `tokens.css` went on naming "Inter" with no
/// `@font-face` behind it and every Dioxus consumer silently fell through to
/// `ui-sans-serif`. It stays here until there is somewhere for it to go.
#[component]
