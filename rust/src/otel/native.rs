...
/// ```
pub fn telemetry<S>(config: &Config) -> Option<(Telemetry, OtelLayers<S>)>
where
...
// tonic's `Interceptor` contract fixes the `Result<Request<()>, Status>` shape;
// the large `Status` Err is not ours to box.
...
#[allow(clippy::result_large_err)] // same tonic `Interceptor` contract as above
